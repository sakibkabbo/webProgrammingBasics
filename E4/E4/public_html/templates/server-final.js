...
app.delete('/restaurants/:id/:resv_id', (req, res) => {
	...
	Restaurant.findById(id).then((restaurant) => {
		if (!restaurant) {
			...
			rst = restaurant.reservations
			let removed_doc = rst.id(resv_id).remove();
			if (!removed_doc){
				res.status(404).send()
			}else{
				restaurant.update({$set: {reservations: rst}}, {new: true}).then((status) => {
...
app.patch('/restaurants/:id/:resv_id', (req, res) => {
	...
	Restaurant.findById(id).then((restaurant) => {
		if (!restaurant) {
			...
			rst = restaurant.reservations
			let removed_doc = rst.id(resv_id);
			if (!removed_doc){
				res.status(404).send()
			}else{
				log('removed_doc')
				log(removed_doc)
				removed_doc.time = req.body.time
				removed_doc.people = req.body.people
				log(removed_doc)
				restaurant.update({$set: {reservations: rst}}, {new: true}).then((status) => {
					if (!status) {
						res.status(404).send()
